  ...
  module Archives
    class Archive < Jekyll::Page
      ...
      def initialize(site, title, type, posts)
		...
		#puts "File.basename relative_path is #{File.basename(relative_path)}"
		#puts "ext is #{File.extname(relative_path)}"
        @ext  = ".html" #File.extname(relative_path)
		#puts "path is #{relative_path}"
        @path = relative_path
		#puts "name is #{File.basename(relative_path, @ext)}"
